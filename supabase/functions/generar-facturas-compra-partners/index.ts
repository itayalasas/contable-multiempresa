  ...
  try {
    ...
    for (const [pid, comisionesPartner] of comisionesPorPartner) {
      try {
        ...
        // VERIFICAR SI YA EXISTE UNA FACTURA POR PAGAR PENDIENTE PARA ESTE PROVEEDOR
        ...
        if (facturaExistente) {
          ...
          const { data: facturaActualizada, error: updateError } = await supabase
            .from('facturas_por_pagar')
            .update({
              monto_subtotal: nuevoSubtotal,
              monto_impuestos: nuevoIVA,
              monto_total: nuevoTotal,
              saldo_pendiente: nuevoSaldo,
              fecha_modificacion: new Date().toISOString(),
              observaciones: `${facturaExistente.observaciones}\nFactura de compra: ${facturaCompra.serie}-${facturaCompra.numero_factura}`,
            })
          ...
          cuentaPorPagar = facturaActualizada;

          console.log(`✅ Factura por pagar actualizada: ${cuentaPorPagar.numero} - Nuevo total: $${nuevoTotal.toFixed(2)}`);

        } else {
          // CREAR NUEVA FACTURA POR PAGAR
          const { data: nuevaFactura, error: cuentaError } = await supabase
            .from('facturas_por_pagar')
            .insert({
              empresa_id: empresaId,
              proveedor_id: proveedorId,

/*
  # Agregar tracking de asiento contable a facturas_venta

  1. Nuevas Columnas
    - `asiento_contable_id` (uuid) - Referencia al asiento generado
  ...
  2. Propósito
    - Permitir tracking del estado de generación del asiento contable
...
ADD COLUMN IF NOT EXISTS asiento_contable_id uuid REFERENCES asientos_contables(id),
...
CREATE INDEX IF NOT EXISTS idx_facturas_sin_asiento 
ON facturas_venta(empresa_id, asiento_generado) 
WHERE asiento_generado = false;

-- Crear índice para búsquedas de facturas con error
CREATE INDEX IF NOT EXISTS idx_facturas_error_asiento 
ON facturas_venta(empresa_id) 

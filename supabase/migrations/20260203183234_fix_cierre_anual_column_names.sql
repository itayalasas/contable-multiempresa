/*
  # Fix Column Names for Annual Closing Functions

  1. Fixes
    - Corrige referencias de ejercicio_fiscal_id a ejercicio_id
    - Corrige referencias de periodo_contable_id a periodo_id
    - Actualiza todas las funciones de cierre anual
*/

-- Función para validar cierre secuencial (CORREGIDA)
...
BEGIN
  ...
  -- Contar períodos abiertos anteriores en el mismo ejercicio (CORREGIDO: ejercicio_id)
  SELECT COUNT(*) INTO v_count
  FROM periodos_contables
  WHERE ejercicio_id = v_periodo.ejercicio_id
...
-- Función para validar si un ejercicio está listo para cierre (CORREGIDA)
...
DECLARE
  ...
  v_abiertos INTEGER;
BEGIN
  -- Contar períodos (CORREGIDO: ejercicio_id)
  SELECT COUNT(*) INTO v_total
  FROM periodos_contables
  WHERE ejercicio_id = p_ejercicio_id;
...
-- Función para cerrar un ejercicio fiscal (CORREGIDA)
...
BEGIN
  ...
  -- Marcar todos los períodos como cerrado_definitivo (CORREGIDO: ejercicio_id)
  FOR v_periodo IN
  ...
  -- Registrar el cierre en historial (CORREGIDO: periodo_id y ejercicio_id)
...
-- Función para reabrir un ejercicio fiscal (CORREGIDA)
...
BEGIN
  ...
  -- Cambiar períodos cerrado_definitivo a cerrado (CORREGIDO: ejercicio_id)
  UPDATE periodos_contables
  SET estado = 'cerrado'
  WHERE ejercicio_id = p_ejercicio_id
  ...
  -- Registrar en historial (CORREGIDO: periodo_id y ejercicio_id)
  INSERT INTO cierres_contables (
    periodo_id,
    ejercicio_id,
    empresa_id,
